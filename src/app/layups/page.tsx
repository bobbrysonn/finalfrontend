...
export default async function Page() {
    /* Fetch best courses */
    const resp = await fetch(`${process.env.API_ROOT}/api/courses/?layup=30`);
    ...
    return (
        <main className="container mx-auto p-4">
            ...
            {/* List of courses */}
            <ListLayups courses={courses}/>
        </main>
    );
}

function ListLayups({courses}: { courses: Course[] }) {
    return (
        <Card>
            <CardHeader>
                <CardTitle>Layups</CardTitle>
                <CardDescription>
                    Viewing courses sorted by layup votes
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Table>
